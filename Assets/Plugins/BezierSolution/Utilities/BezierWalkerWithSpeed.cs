...
namespace BezierSolution
{
	public class BezierWalkerWithSpeed : MonoBehaviour
	{
		...
		public enum Orient
		{
			//green forward, blue right
			common,

			//blue forward, green right
			swapped
		}
		...
		public Orient orientation = Orient.common;
		...
		void Update()
		{
			...
			if( lookForward )
			{
                ...
                //Vector3 savedRight = transform.right;

				if (orientation == Orient.common)
				{
					var newU = (targetPos - transform.position).normalized;
					float ang = Vector3.SignedAngle(transform.up, newU, transform.right);
				...
				{
					var newU = (targetPos - transform.position).normalized;
					float ang = Vector3.SignedAngle(transform.forward, newU, transform.right);

					transform.Rotate(0, 0, -ang, Space.World);
				}
				//transform.localEulerAngles = new Vector3(ang, transform.localEulerAngles.y, transform.localEulerAngles.z);


                //transform.up = spline.GetTangent(progress) - transform.position;
                //transform.right = savedRight;
