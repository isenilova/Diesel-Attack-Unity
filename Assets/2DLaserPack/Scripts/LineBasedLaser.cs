...
namespace TwoDLaserPack
{
    public class LineBasedLaser : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            OnLaserHitTriggered += LineBasedLaser_OnLaserHitTriggered;
        }

        private void LineBasedLaser_OnLaserHitTriggered(RaycastHit2D hitInfo)
        {
            //Debug.Log(hitInfo.collider);
            if (hitInfo.collider.tag == "Player")
            {
                LevelManager.Instance.KillPlayer(hitInfo.collider.gameObject.GetComponent<Character>());
            }
        }
