	...
	public class Health : MonoBehaviour
	{
		...
		public virtual void Damage(int damage,GameObject instigator, float flickerDuration, float invincibilityDuration)
		{
			...
			if (CurrentHealth <= 0)
			{
                if (explodeOnDeath)
                {
                    //SpriteSlicer2DDemoManager.instance.ExplodeSprite(gameObject);
                }
        ...
        {
            _character.gameObject.layer = LayerMask.NameToLayer("Invulnerable");
		...
		{
            //we ask for if its main character and boss level we restart it
            Debug.Log("revived");
            if (_character != null)
            {
