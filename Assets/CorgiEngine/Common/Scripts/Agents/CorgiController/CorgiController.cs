...
{	
	...
	public class CorgiController : MonoBehaviour 
	{
		...
		public CorgiControllerParameters _overrideParameters;
		...
		protected virtual void ApplyGravity()
		{
			...
			if (_gravityActive)
			{
                //check here if we are above crate 

                /*
                RaycastHit2D rh;
                rh = Physics2D.Raycast(transform.position, Vector2.down, 0.1f, 1 << LayerMask.NameToLayer("Platforms"));
                if (rh.collider == null || GetComponent<Health>().Invulnerable == true)
                */
		...
		protected virtual void ComputeNewSpeed()
		{
			...
			if (Time.deltaTime > 0)
			{
                //Debug.Log(GetComponent<Character>().MovementState.CurrentState);
                //if (GetComponent<Character>().MovementState.CurrentState == CharacterStates.MovementStates.Dangling)
                //Debug.Log(State.WasGroundedLastFrame);
