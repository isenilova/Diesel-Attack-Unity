...
{	
	...
	{	
		...
		{
            if (LevelControl2D.instance != null && !LevelControl2D.instance.isLoaded && LevelControl2D.instance.doInst) return;
            if (PlayerInfo.instance != null) PlayerPrefabs[0] = PlayerInfo.instance.player;
            isLoaded = true;

			return;
            Debug.Log(PlayerPrefabs[0]);
            //Debug.Log(PlayerInfo.instance.player);
		...
		protected virtual void InstantiatePlayableCharacters()
		{
			...
			{ 
				foreach (Character playerPrefab in PlayerPrefabs)
				{
                    ...
                    {
                        newPlayer.transform.position = CurrentCheckPoint.transform.position;
                    }
		...
		public virtual void Update()
		{
            if (!isLoaded)
            {
                if (LevelControl2D.instance != null && !LevelControl2D.instance.isLoaded && LevelControl2D.instance.doInst)
                {
                    return;
                }

                if (LevelControl2D.instance == null || (LevelControl2D.instance != null && !LevelControl2D.instance.doInst))
                {
                    Awake();
                    Start();
                    isLoaded = true;
                ...
                {
                    PlayerPrefabs[0] = PlayerInfo.instance.player;
                }
		...
		{
            if (player.gameObject.layer == LayerMask.NameToLayer("Invulnerable"))
            {
                return;
            }
			...
			else
			{
                ...
                if (player.GetComponent<CharacterAbility>()._movement.CurrentState == CharacterStates.MovementStates.Dashing)
                {
                    player.GetComponent<CharacterAbility>()._movement.ChangeState(CharacterStates.MovementStates.Idle);
                }

                player.GetComponent<CorgiController>().ResetParameters();
                player.GetComponent<CharacterGravity>()._gravityOverridden = false;
	    ...
	    protected virtual IEnumerator SoloModeRestart(Character player = null)
		{
			if (PlayerPrefabs.Count() <= 0)
			{
				yield break;
			...
			if (CurrentCheckPoint != null)
			{
                ...
                if (player != null && player.GetComponent<Cloned>() != null)
                {
                    var ft = GameObject.FindGameObjectWithTag("hero");
                    player.transform.position = ft.transform.position;
                    ...
                    for (int u = 0; u < gos.Length; u++)
                    {
                        if (gos[u] != player.gameObject)
                        {
                            player.GetComponent<CorgiController>().PlatformMask = gos[u].GetComponent<CorgiController>().PlatformMask;
                        }
                    }
                }
                else
